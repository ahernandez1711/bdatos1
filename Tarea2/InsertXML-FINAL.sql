USE test
--SCRIPT DE INSERTAR XML EN SQL SERVER 2012
BEGIN
	SET NOCOUNT ON
...
BEGIN TRY	
	...
	<error Codigo="50012" Descripcion="Error de base de datos"/>
	<error Codigo="50013" Descripcion="Error de base de datos"/>
	</Error>
	</Datos>';

--PARA CADA DATO CON ID FIJO EN EL XML HAY QUE PRENDER LA BANDERA DE IDENTITY_INSERT EN ON Y LUEGO APAGARLA
...
--
/* deletes existen porque si se inserta un valor FIJO IDENTITY dentro de una tabla con datos ya da error
hay que limpiar las tablas en bd si se quiere reutilizar el script
...
--LISTA DE INSERTS
	...
	SELECT 
		...
		TipoMovimiento.value('@Nombre', 'NVARCHAR(100)') AS Nombre,
		TipoMovimiento.value ('@TipoAccion', 'NVARCHAR(100)') AS TipoAccion
	...
	SELECT
		...
		Usuario.value('@Nombre', 'NVARCHAR(100)') AS Username,
		Usuario.value('@Pass', 'NVARCHAR(4)') AS Pass
	...
	--AQUI TERMINAN LOS CATALOGOS

	--AQUI COMIENZAN LOS DATOS DE LOS EMPLEADOS Y MOVIMIENTOS
	--AQUI HAY QUE INSERTAR BASANDONOS EN EL ORDEN DE FECHA : USAR WHILE
	DECLARE @CurrentDate DATE
	DECLARE @EndDate DATE
	--DECLARAR VARIABLES DE FECHA PARA CREAR UN CICLO PARA COMPARAR LAS FECHAS DE INSERCION

	SELECT @CurrentDate = MIN(Empleado.value('@FechaContratacion', 'DATE')),
		@EndDate = MAX(Empleado.value('@FechaContratacion', 'DATE'))
	FROM @xmlData.nodes('/Datos/Empleados/empleado') AS T(Empleado)
	--Seteamos las fechas con la mas vieja y mas nueva
	---empleados
	...
	BEGIN
		...
		SELECT
			...
			Empleado.value('@ValorDocumentoIdentidad', 'INT') AS ValorDocumentoIdentidad,
			Empleado.value('@Nombre', 'NVARCHAR(100)') AS Nombre,
			Empleado.value('@FechaContratacion', 'DATE') AS FechaContratacion,
			Empleado.value('@SaldoVacaciones', 'MONEY') AS SaldoVacaciones,
			Empleado.value('@EsActivo', 'BIT') AS EsActivo
		FROM @xmlData.nodes('/Datos/Empleados/empleado') AS T(Empleado)
		WHERE Empleado.value('@FechaContratacion', 'DATE') = @CurrentDate

		SET @CurrentDate = DATEADD(DAY, 1, @CurrentDate) --Este es el iterador para fechas, no se puede simplemente sumar 1 a la fecha porque no es entero
		--hay que utilizar DATEADD (D/M/Y, #, ValorFecha)
	END


	---Movimientos
	SELECT @CurrentDate = MIN(Movimiento.value('@Fecha', 'DATE')),
       @EndDate = MAX(Movimiento.value('@Fecha', 'DATE'))
	...
	BEGIN
		...
		--ALIASING: E - Empleado, U- Usuario
			...
			E.SaldoVacaciones +1 AS NuevoSaldo, --Saldo Vacaciones de la tabla Empleado 
			U.Id AS idPostByUser, -- No sabemos porque esta marcado como ID pero en el xml es un varchar pero igual esta aqui
			Movimiento.value('@PostInIP', 'VARCHAR(64)') AS PostInIP,
			Movimiento.value('@PostTime', 'DATETIME') AS PostTime
		FROM @xmlData.nodes('/Datos/Movimientos/movimiento') AS T(Movimiento)
		INNER JOIN dbo.Empleado E --ACTUALIZACION DE SALDOS MEDIANTE JOIN DE NUEVOSALDO Y SALDOVACACIONES
			ON E.ValorDocumentoIdentidad = Movimiento.value('@ValorDocId', 'INT') --atribuido al empleado con # documento
		LEFT JOIN dbo.Usuario U --
			ON U.Username = Movimiento.value('@PostByUser', 'NVARCHAR(100)') -- 
		WHERE Movimiento.value('@Fecha', 'DATE') = @CurrentDate;

		
		UPDATE E --Actualizar el saldo en la tabla Empleado luego del movimiento
		SET E.SaldoVacaciones = M.NuevoSaldo
		FROM dbo.Empleado E
		INNER JOIN (
			SELECT 
				idEmpleado, 
				MAX(Fecha) AS LatestDate --aliasing de ultima fecha
			FROM dbo.Movimiento
			GROUP BY idEmpleado
		) LatestMovimiento ON E.id = LatestMovimiento.idEmpleado --aliasing de ultimo movimiento
		INNER JOIN dbo.Movimiento M
			ON M.idEmpleado = LatestMovimiento.idEmpleado --join de valores
			AND M.Fecha = LatestMovimiento.LatestDate; --utilizando la fecha mas reciente


		SET @CurrentDate = DATEADD(DAY, 1, @CurrentDate) --incrementar la iteracion de fechas
	END
	

--no sabemos porque esta tabla esta en el xml y no en la especificacion del proyecto pero la tenemos igual
	...
	SELECT
		...
		Feriado.value('@Descripcion', 'VARCHAR(100)') AS Descripcion
	FROM @xmlData.nodes('/Datos/Feriados/feriado') AS T(Feriado);
